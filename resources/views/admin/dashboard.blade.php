...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:16px;margin-bottom:24px">
    ...
    <div class="card" style="text-align:center;padding:24px">
        <div style="font-size:32px;font-weight:700;color:var(--leaf)">{{ $stats['cats'] }}</div>
        <div style="color:#666;margin-top:8px">Mèo</div>
    </div>
    <div class="card" style="text-align:center;padding:24px">
    ...
    <div class="card" style="text-align:center;padding:24px">
        <div style="font-size:32px;font-weight:700;color:#f59e0b">{{ $stats['bookings'] }}</div>
        <div style="color:#666;margin-top:8px">Booking</div>
    </div>
    <div class="card" style="text-align:center;padding:24px">
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:24px">
    <div class="card">
        ...
        <p><strong>Booking mèo:</strong> {{ number_format($stats['revenue_bookings'], 0, ',', '.') }}đ</p>
        <p style="font-size:18px;font-weight:700;margin-top:12px;color:var(--coffee)">
            Tổng: {{ number_format($stats['revenue_orders'] + $stats['revenue_bookings'], 0, ',', '.') }}đ
        </p>
    </div>
    <div class="card">
        <h3 style="margin-bottom:16px;color:#f59e0b">Chờ xử lý</h3>
        <p><strong>Đơn hàng:</strong> {{ $stats['pending_orders'] }}</p>
        <p><strong>Booking:</strong> {{ $stats['pending_bookings'] }}</p>
    </div>
</div>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
    ...
    <div class="card">
        <h3 style="margin-bottom:16px">Booking gần đây</h3>
        @forelse($recentBookings as $booking)
            <div style="padding:8px 0;border-bottom:1px solid var(--border)">
                <strong>#{{ $booking->id }}</strong> - {{ $booking->user->username }} - {{ $booking->cat->name }} - {{ number_format($booking->amount, 0, ',', '.') }}đ
            </div>
        @empty
            <p style="color:#999">Chưa có booking nào</p>
        @endforelse
    </div>
</div>
@endsection
